...
void man(char * name)
{
    ...
    {
        printf("Description : Used to list information of files\n");
        printf("Usage : ls\n");
    }
    ...
    {
        printf("Used to change file offset\n");
...
void DisplayHelp()
{
    ...
    printf("read : To read all contents into file\n");
...
int CreateFile(char * name,int permission)
{
    ...
    UFDTArr[i].ptrfiletable = (PFILETABLE)malloc(sizeof(FILETABE));
...
void CloseFileByName(int fd) //this given on line 395 pdf
{
...
int CloseFileByName(char * name)   //Here comes error on main.661  //this given on line 402 in pdf
{
    int i = 0;
    ...
    while(1)
    {
        ...
        else if(count == 3)
        {
            ...
            else if(strcmp(command[0],"open") == 0)
            {
                ...
                if(ret >= 0)
                    printf("File is successively opened with file descriptor %d\n",ret);
                if(ret == -1)
                    printf("ERROR: Incorrect parameters\n");
                if(ret == -2)
